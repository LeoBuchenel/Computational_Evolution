...
Ecosystem::Ecosystem(Grid* grid, Zone const& animal_zone, Zone const& plant_zone,
								...
								: animal_zone(animal_zone), plant_zone(plant_zone), grid(grid)
{
...
void Ecosystem::move()
{
								for(auto const& obj:animal_list) {
									if(obj->isAlive()){
										for(unsigned int i(0); i < obj->get_nb_moves(); ++i){
...
{
	double alpha = 0.05;
	unsigned int nbFood_new(alpha*grid->getNbFood());
	unsigned int plant_zone_size(plant_zone.size());
	
	for(unsigned int i(0); i < nbFood_new; ++i){
		unsigned int n(rand() % plant_zone_size);
		plant_zone[n] -> addFood();	
	}
}
